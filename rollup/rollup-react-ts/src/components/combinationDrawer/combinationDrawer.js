...
const CombinationDrawer = (props, ref) => {
    ...
    newList = newList.map((item, key) => {
      ...
      if (title === 'transition') {
        ...
        if (titles && title) {
          ...
          if (item.fatherSonConnection === index) {
            index = Number(index);
            if (title) {
              const titleList = [];
  ...
  const newListDom = (list) => {
    ...
    list.map((item, index) => {
      ...
      newList.push(
        <div
          key={`${index}`}
          className={`${styles.transitions} ${styles.drawerContentItemBox} `}
          style={obj}
        >
          {index === 0 ? (
            <div className={`${styles.closure}`}>
              {!item.state ? (
                <ArrowLeftOutlined onClick={() => onUnfold(index)} className={styles.anticonLeft} />
              ) : (
                itemShow.length >= 1 && (
                  <ArrowRightOutlined
                    onClick={() => closure(index, 'stretch')}
                    className={styles.anticonLeft}
                  />
                )
              )}
              <CloseOutlined onClick={setDrawerShows} className={styles.anticon} />
            </div>
          ) : (
            <div className={`${styles.closure}`}>
              {item.title}
          ...
          {item.value}
          {itemShow[index] && <div className={`${styles.pointerEvents}`}></div>}
        </div>,
      );
    });
    return newList;
  ...
  return (
    <div ref={drawers}>
      <div
        className={`${styles.transitions} ${styles.drawer} ${drawerShow && styles.translateXShow}`}
