  ...
  const selector = (e, key, value) => {
    ...
    for (let i = 0; i < dom.length; i++) {
      ...
      if (i < key) {
        ...
        dom[i].style.transform = `translate3d(${Math.pow(key - i, 2) * inclination
          }px, 0px, 0px)`;
      } else {
        if (setSelectedKey !== null) {
          if (i - key === 1 && key < id) {
        ...
        dom[i].style.transform = `translate3d(${Math.pow(i - key, 2) * inclination
          }px, 0px, 0px)`;
      }
    }
    indexKey = key;
            ...
            {dataList.map((item, index) => {
              return (
                <span
                  ...
                  style={{
                    transform: `translate3d(${inclination * Math.pow(index, 2)
                      }px, 0px, 0px)`,
                    top: `calc(50% - ${itemHeight}px)`,
...
export const TabSwitchPage = TabSwitch;
