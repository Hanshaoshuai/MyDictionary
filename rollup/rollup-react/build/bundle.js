...
var CombinationDrawer = function CombinationDrawer(props, ref) {
  var list = props.list,
      drawerShow = props.drawerShow,
      setDrawerShow = props.setDrawerShow,
      filters = props.filters,
      titles = props.titles,
      _props$redundantWidth = props.redundantWidth,
      redundantWidth = _props$redundantWidth === void 0 ? 34 : _props$redundantWidth,
  ...
  React.useImperativeHandle(ref, function () {
    return {
      getInfo: function getInfo(index) {
        //父组件调用了子组件需要处理的数据
  ...
  React.useEffect(function () {
    var index = filters.index,
        title = filters.title;
    onfilters(index, title);
  }, [filters]);
  var drawers = React.useCallback(function (node) {
    if (node !== null) {
      var domList = _toConsumableArray(node.children).filter(function (i, index) {
        return index > 0;
      });
  ...
  var setDrawerShows = function setDrawerShows() {
    itemShow = [];
    itemShowList = [];
    itemShowList1 = [];
  ...
  var itemLast = function itemLast(key, itemLength, item, newList) {
  ...
  var onfilters = function onfilters(index, title) {
    // if (!index && !title) return;
    var newList = _toConsumableArray(newData);

    var indexN = Number(index);
    ...
    var stateKeyId = 0;
    var indexWidth = 0;
    var transitionFalseId = false;
    var notExpandedIndex = 0;
    var notExpandedIndexId = null;
    var keyId = (indexN + 1).toString();
    newList.map(function (item, key) {
      if (key === 0) {
        item.show = true;
      }
    ...
    newList = newList.map(function (item, key) {
      ...
      if (title === 'transition') {
        ...
        if (key === indexN) {
          ...
          item.state = true;
          var _obj = newList[key - 1];
          item.transition = item.notExpanded ? item.width + _obj.transition - redundantWidth : _obj.transition;
        }
        ...
        if (titles && title) {
            ...
            if (title) {
              ...
              titleList.push(jsx("span", {
                ...
                onClick: function onClick() {
                  return closure(indexN.toString());
                }
              }, newList[index].title && jsx("span", null, " / "), title));
          ...
          if (key === index) {
            if (!item.notExpanded && index - 1 >= 0) {
              if (notExpandedIndexId !== null && (notExpandedIndexId === 0 || notExpandedIndexId < key)) {
    ...
    if (notExpandedId > 2) {
      // console.log('超过2个');
      newList = newList.map(function (item, key) {
        if (key === Number(keyId) && Number(keyId) !== newList.length - 1 && item.notExpanded && itemShowList1[key]) {
          item.notExpanded = false;
          item.transition = item.transition - item.width + redundantWidth;
  ...
  var onUnfold = function onUnfold(e) {
    onfilters(e, 'transition');
  };

  var closure = function closure(fatherSonConnection, stretch) {
    onfilters(fatherSonConnection, stretch);
  };

  var newListDom = function newListDom(list) {
    var newList = [];
    list.map(function (item, index) {
      var obj = {
        width: "".concat(item.width ? item.width + 'px' : 'auto'),
        zIndex: 190 - index
      };

      if (drawerShow && !item.state) {
        obj.transform = "translateX(".concat(-redundantWidth * (index + 1), "px)");
      }

      if (index === 0 && drawerShow && item.state) {
        obj.transform = "translateX(".concat(-100, "%)");
      } else {
        if (drawerShow && item.state) {
          obj.transform = "translateX(".concat(-item.transition - redundantWidth * index + 1, "px)");
        }
      }
      ...
      newList.push(jsx("div", {
        key: "".concat(index),
        className: "transitions drawerContentItemBox",
        style: obj
      }, index === 0 ? jsx("div", {
        className: "closure"
      }, !item.state ? jsx(icons.ArrowLeftOutlined, {
        onClick: function onClick() {
          return onUnfold(index);
        },
        className: "anticonLeft"
      }) : itemShow.length >= 1 && jsx(icons.ArrowRightOutlined, {
        onClick: function onClick() {
          return closure(index, 'stretch');
        },
        className: "anticonLeft"
      }), jsx(icons.CloseOutlined, {
        onClick: setDrawerShows,
        className: "anticon"
      })) : jsx("div", {
        className: "closure"
      }, item.title, itemShow[index] && jsx(React__default["default"].Fragment, null, !item.state || !item.notExpanded ? jsx(icons.ArrowLeftOutlined, {
        onClick: function onClick() {
          return onUnfold(index);
        },
        className: "anticonLeft"
      }) : jsx(icons.ArrowRightOutlined, {
        onClick: function onClick() {
          return closure(index, 'stretch');
        },
        className: "anticonLeft"
        ...
        onClick: function onClick() {
          return closure((index - 1).toString());
        },
        className: "anticon"
      })), item.value, itemShow[index] && jsx("div", {
  ...
  return jsx("div", {
    ...
    className: "transitions drawer ".concat(drawerShow && 'translateXShow'),
    onClick: setDrawerShows
  }), newListDom(newData));
};

var combinationDrawer = /*#__PURE__*/React.forwardRef(CombinationDrawer);

exports.CombinationDrawer = combinationDrawer;
